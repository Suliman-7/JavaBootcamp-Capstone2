...
public class CoordinatorController {
    ...
    @PostMapping("/post")
    public ResponseEntity addCoordinator(@Valid @RequestBody Coordinator coordinator, Errors errors) {
        if (errors.hasErrors()) {
    ...
    public ResponseEntity updateCoordinator(@PathVariable int id , @Valid @RequestBody Coordinator coordinator, Errors errors) {
        if (errors.hasErrors()) {
            String message = errors.getFieldError().getDefaultMessage();
            return ResponseEntity.status(400).body(message);
        }
